...
var CORE = (function () {
  ...
  var PRESETS = {
    ...
    everything: { label: 'Everything', ids: MODULES.filter(function (m) { return !m.support; }).map(function (m) { return m.id; }) }
  };
  ...
  function proxyExclusions(st) {
    ...
    selectedModules(st).forEach(function (m) {
      ...
      } else if (m.dockerSocket && st.dockerSocket) {
  ...
  function buildCaddyfile(st) {
    ...
    selectedModules(st).forEach(function (m) {
      ...
      if (m.dockerSocket && st.dockerSocket) return;
  ...
  function validate(st) {
    ...
    // overlapping jobs are a warning, never a block
    var byJob = {};
    mods.forEach(function (m) { (byJob[m.job] = byJob[m.job] || []).push(m.name); });
    Object.keys(byJob).forEach(function (job) {
      if (byJob[job].length > 1) {
        warnings.push(byJob[job].join(' and ') + ' do the same job. Running both works, but you probably want one.');
      }
    });
  ...
  /* ---------- install.sh ----------
     POSIX sh, no bash 4 syntax, because macOS ships bash 3.2. */

  function buildInstallSh(st) {
    var ctx = makeCtx(st);
    var out = '#!/bin/sh\n';
    out += '# install.sh - generated by Modular Workspace Server\n';
    out += '# POSIX sh on purpose: macOS ships bash 3.2, so no associative arrays,\n';
    out += '# no mapfile, no ${var^^}.\n';
    out += 'set -eu\n\n';
    out += 'if ! command -v docker >/dev/null 2>&1; then\n';
    out += '  echo "docker not found on PATH" >&2\n  exit 1\nfi\n\n';

    if (!ctx.layout.named) {
      out += 'echo "Creating storage directories"\n';
      var dirs = {};
      selectedModules(st).forEach(function (m) {
        if (m.deploy !== 'service') return;
        var part = m.compose(ctx);
    ...
    out += 'if [ ! -f .env ]; then\n';
    out += '  echo "No .env found. Copy .env.example to .env and fill it in first." >&2\n';
    out += '  exit 1\nfi\n\n';
    out += 'if grep -q "^[A-Z_]*=CHANGEME$" .env; then\n';
    out += '  echo "Warning: .env still contains CHANGEME values:" >&2\n';
    out += '  grep -n "=CHANGEME$" .env >&2\n';
    out += 'fi\n\n';
    out += 'echo "Starting the stack"\n';
    out += 'docker compose up -d\n';
    out += 'echo\n';
    out += 'echo "Running. Check progress with: docker compose logs -f"\n';
    return out;
  }
  ...
  return {
    ...
    buildInstallSh: buildInstallSh, dbInit: dbInit,
